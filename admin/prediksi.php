        ...
        <div class="row">
          <div class="col-md-12">
            <div class="content">
                    ...
                    <div class="form-group">
                      ...
                      <?php if ($_SESSION['level'] == 'admin2') : ?>
                        <?php if (isset($_POST['cari'])) {
                          ...
                          // Convert bulan and tahun to integers
                          $bulan = (int)$bulan;
                          $tahun = (int)$tahun;
                          ...
                          // Generate the URL with the incremented month
                          $url = "?id_kategori={$id_kategori}&bulan={$bulan}&tahun={$tahun}";
                        ?>
                          
                          <a href="cetak_cukup.php<?= $url ?>" target="_blank()"><button data-toggle="tooltip" data-placement="top" title="Cetak Data Cukup" type="button" class="btn btn-danger btn-sm"><i class="fa fa-print"></i> Cetak Prediksi Cukup</button></a>
                          <a href="cetak_kurang.php<?= $url ?>" target="_blank()"><button data-toggle="tooltip" data-placement="top" title="Cetak Data Kurang" type="button" class="btn btn-danger btn-sm"><i class="fa fa-print"></i> Cetak Prediksi Kurang</button></a>
                        <?php } ?>
                      <?php endif; ?>
                    </div>
                  </div>
              ...
              if (isset($_POST['cari'])) {
                if (isset($_POST['id_kategori']) && isset($_POST['bulan']) && isset($_POST['tahun'])) {
                  ...
                  <tbody>
                    ...
                    if (isset($_POST['cari'])) {
                      ...
                      if ($_POST['bulan'] != 0 && !empty($_POST['tahun'])) {
                        ...
                        // Perhitungan bulan sebelumnya
                        $previousMonth = $_POST['bulan'] - 1;
                        $previousYear = $_POST['tahun'];
                        if ($previousMonth == 0) {
                          $previousMonth = 12;
                          $previousYear -= 1;
                        }
                        $whereBulanPrevious = "AND MONTH(d.tanggal_jual) = '$previousMonth' AND YEAR(d.tanggal_jual) = '$previousYear'";
                      } else {
                        $whereBulan = "";
                        $whereBulan2 = "";
                        $whereBulanPrevious = "";
                      ...
                      if ($qryBrg > 0) {
                        ...
                        $query = mysqli_query($koneksi, "SELECT * FROM data_barang WHERE id_kategori = '$_POST[id_kategori]' ORDER BY id_barang DESC");
                      }
                      while ($data = mysqli_fetch_array($query)) {
                        ...
                        if ($num_rekap > 0) {
                          ...
                          $jumlah_cukup_prev = 0;
                          $jumlah_kurang_prev = 0;
                          $nama_barang_cukup_prev = 0;
                          $nama_barang_kurang_prev = 0;
                          $kategori_cukup_prev = 0;
                          $kategori_kurang_prev = 0;
                          $ukuran_cukup_prev = 0;
                          $ukuran_kurang_prev = 0;
                          $harga_cukup_prev = 0;
                          $harga_kurang_prev = 0;

                          $total_stok_cukup_prev = 0;
                          $total_stok_kurang_prev = 0;
                          $total_jual_cukup_prev = 0;
                          $total_jual_kurang_prev = 0;
                          $total_jual_cukup_prev = 0;
                          $total_jual_kurang_prev = 0;

                          $total_harga_cukup_prev = 0;
                          $total_harga_kurang_prev = 0;
                          ...
                          // Mengambil data penjualan bulan sebelumnya
                          $query_rekap_previous = mysqli_query($koneksi, "SELECT a.*, b.*, c.*, d.* FROM data_rekap a, data_barang b, data_kategori c, data_penjualan d WHERE a.id_barang = '$data[id_barang]' AND c.id_kategori = '$_POST[id_kategori]' AND a.id_penjualan = d.id_penjualan $whereBulanPrevious ORDER BY a.id_rekap DESC");
                          while ($rekap_previous = mysqli_fetch_array($query_rekap_previous)) {
                            if ($rekap_previous['hasil'] == 'cukup') {
                              $jumlah_cukup_prev++;
                              $total_stok_cukup_prev += $rekap_previous['stok'];
                              $total_jual_cukup_prev += $rekap_previous['stok_terjual'];
                              $total_harga_cukup_prev += $rekap_previous['harga'];
                              if ($rekap_previous['nama_barang'] == $data['nama_barang']) $nama_barang_cukup_prev++;
                              if ($rekap_previous['kategori'] == $rekap_previous['kategori']) $kategori_cukup_prev++;
                              if ($rekap_previous['ukuran'] == $data['ukuran']) $ukuran_cukup_prev++;
                            } else if ($rekap_previous['hasil'] == 'kurang') {
                              $jumlah_kurang_prev++;
                              $total_stok_kurang_prev += $rekap_previous['stok'];
                              $total_jual_kurang_prev += $rekap_previous['stok_terjual'];
                              $total_harga_kurang_prev += $rekap_previous['harga'];
                              if ($rekap_previous['nama_barang'] == $data['nama_barang']) $nama_barang_kurang_prev++;
                              if ($rekap_previous['kategori'] == $rekap_previous['kategori']) $kategori_kurang_prev++;
                              if ($rekap_previous['ukuran'] == $data['ukuran']) $ukuran_kurang_prev++;
                            }
                          }
                          ...
                          // bulan sebelumnya
                          $total_prev = $jumlah_cukup_prev + $jumlah_kurang_prev;

                          if ($total_prev > 0) {
                            $p_cukup_prev = $jumlah_cukup_prev / $total_prev;
                            $p_kurang_prev = $jumlah_kurang_prev / $total_prev;
                          } else {
                            $p_cukup_prev = 0;
                            $p_kurang_prev = 0;
                          }

                          $p_nama_cukup_prev = ($jumlah_cukup_prev > 0) ? $nama_barang_cukup_prev / $jumlah_cukup_prev : 0;
                          $p_nama_kurang_prev = ($jumlah_kurang_prev > 0) ? $nama_barang_kurang_prev / $jumlah_kurang_prev : 0;

                          $p_kategori_cukup_prev = ($jumlah_cukup_prev > 0) ? $kategori_cukup_prev / $jumlah_cukup_prev : 0;
                          $p_kategori_kurang_prev = ($jumlah_kurang_prev > 0) ? $kategori_kurang_prev / $jumlah_kurang_prev : 0;

                          $p_ukuran_cukup_prev = ($jumlah_cukup_prev > 0) ? $ukuran_cukup_prev / $jumlah_cukup_prev : 0;
                          $p_ukuran_kurang_prev = ($jumlah_kurang_prev > 0) ? $ukuran_kurang_prev / $jumlah_kurang_prev : 0;

                          $p_hasil_cukup_prev = $p_cukup_prev * $p_nama_cukup_prev * $p_kategori_cukup_prev * $p_ukuran_cukup_prev;
                          $p_hasil_kurang_prev = $p_kurang_prev * $p_nama_kurang_prev * $p_kategori_kurang_prev * $p_ukuran_kurang_prev;

                          $hasil_prediksi_prev = ($p_hasil_cukup_prev > $p_hasil_kurang_prev) ? 'cukup' : 'kurang';

                          // Prediksi harga berdasarkan rata-rata historis
                          $rata2_harga_cukup_prev = ($jumlah_cukup_prev > 0) ? $total_harga_cukup_prev / $jumlah_cukup_prev : 0;
                          $rata2_harga_kurang_prev = ($jumlah_kurang_prev > 0) ? $total_harga_kurang_prev / $jumlah_kurang_prev : 0;

                          $prediksi_harga_prev = ($hasil_prediksi_prev == 'cukup') ? $rata2_harga_cukup_prev : $rata2_harga_kurang_prev;

                          // Prediksi stok dan penjualan berdasarkan rata-rata historis
                          $rata2_stok_cukup_prev = ($jumlah_cukup_prev > 0) ? $total_stok_cukup_prev / $jumlah_cukup_prev : 0;
                          $rata2_stok_kurang_prev = ($jumlah_kurang_prev > 0) ? $total_stok_kurang_prev / $jumlah_kurang_prev : 0;

                          $rata2_jual_cukup_prev = ($jumlah_cukup_prev > 0) ? $total_jual_cukup_prev / $jumlah_cukup_prev : 0;
                          $rata2_jual_kurang_prev = ($jumlah_kurang_prev > 0) ? $total_jual_kurang_prev / $jumlah_kurang_prev : 0;
                          $prediksi_terjual_prev = ($hasil_prediksi_prev == 'cukup') ? round($rata2_jual_cukup_prev) : round($rata2_jual_kurang_prev);
                          ...
                          if ($query_hasil > 0) {
                            ...
                            $simpan = mysqli_query($koneksi, "INSERT INTO data_hasil (id_hasil, id_barang, harga, stok, terjual, terjual_sebelumnya, selisih_stok, jumlah_harga_siapkan, hasil_prediksi, tanggal) VALUES (NULL, '$data[id_barang]', '$prediksi_harga', '$prediksi_stok', '$prediksi_terjual', '$prediksi_terjual_prev', '$selisih_stok', '$jumlah_harga_siapkan', '$hasil_prediksi', '$data[tanggal_update]')");
                    ...
                    ?>
                          ...
                          if (!$showedElseMessage) {
                          ?>
                            <tr>
                              <td class="text-dark" colspan="11" align="center">Data hasil prediksi hanya untuk penjualan bulan selanjutnya.</td>
                            </tr>
                        <?php
                            $showedElseMessage = true;
                          }
                        }
                        ?>
                      <?php
